...
def Pre(frame_):
    ...
    adap = cv2.adaptiveThreshold(gray,255,cv2.ADAPTIVE_THRESH_MEAN_C,cv2.THRESH_BINARY,201,-55)
...
def Classic(frame_, side_tracking, mode, dis, center_old, x):
    ...
    if mode == 0 or mode == 3:
        ...
        for cnt in contours:
            ...
            if (scnt<4000) and (block_w>10) and (block_w<45) and (block_h >50) and (stt<3):
                cv2.drawContours(img,[box2],0,(255,130,171),-1)
                dis_01 = np.sqrt((box2[0][0]-box2[1][0])**2+(box2[0][1]-box2[1][1])**2)
                dis_03 = np.sqrt((box2[0][0]-box2[3][0])**2+(box2[0][1]-box2[3][1])**2)
                #if box[1][1]<box[3][1]:
                if dis_01 > dis_03:
                ...
                if center_low < 380 and center_high>center_low-50:
