...
def Pre(frame_):
    ...
    _,contours,__ = cv2.findContours(closing,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
    for cnt in contours:
...
def Classic(frame_, side_tracking, mode, dis, center_old, x):
    ...
    if mode == 0:
        _,contours,__ = cv2.findContours(frame,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
