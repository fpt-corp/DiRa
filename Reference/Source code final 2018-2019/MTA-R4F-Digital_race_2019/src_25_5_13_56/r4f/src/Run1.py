...
from modules.Route1 import navigator
...
from modules.StopSign import Get_sign_distance
import config as cf
import rospkg 
...
def get_route_message(data):
	...
	print "#"*20
	print "Received route message:", cf.route
	if cf.route != cf.old_route:
		#cf.route = ["1 2 3 4 5"]
		shortest_route_table = [
		["5L", "12", "5R", "3R", "4L", "5L", "5E" ],
		["5L", "21", "5R", "3R", "4L", "5L", "5E" ],

		]
		if route_arr[0] == '1' :
			cf.shortest_route = shortest_route_table[0]
		if route_arr[0] == '2' :
			cf.shortest_route = shortest_route_table[1]
...
def speed_control():
    ...
    if cf.parking and cf.sign_distance>203:
	set_speed(-100)
	time.sleep(1)
	set_speed(0)
	cf.pause = True
...
def get_center(img_):
    img = img_.copy()
    if cf.parking:
    	cf.sign_distance, frame = Get_sign_distance((cf.img).copy(), cv2.resize(cf.depth, (320, 240)))
	cf.img_show2 = frame
...
def show():
    ...
    while cf.running:     
	    ...
	    cv2.putText(img,str(int(cf.angle_checkpoint)),(250,25), font, 1, (255,0,0), 2, 1)
            cv2.putText(img,str(cf.sign_distance),(90,50), font, 0.8, (255,0,0), 2, 1)
	    cv2.putText(img,str(cf.vatcan12),(300,50), font, 0.8, (255,0,0), 2, 1)
	    ...
	    if k == ord('z'):
		cf.lane_choice = (1+cf.lane_choice)%4
...
def listenner():
    ...
    rospy.Subscriber("/route", String, get_route_message, queue_size=1)
    rospy.spin()
